...
const FUN_ASR_HTTP_AUDIO_MIME_TYPES = new Set<string>(DASHSCOPE_FUN_ASR_HTTP_AUDIO_MIME_TYPES);
...
export function createDashScopeFunAsrDirectBody(
  ...
  assertFunAsrDirectAudioMime(cleanMimeType);
  return {
    model,
    input: {
      messages: [
...
export async function transcribeWithDashScopeFunAsrDirect(
  ...
  if (!FUN_ASR_HTTP_AUDIO_MIME_TYPES.has(normalizeMimeType(mimeType))) {
    throw new DailyApiError(
      422,
      "Fun-ASR-Realtime HTTP 接口仅支持 WAV 或 MP3 音频；当前录音无法转换为兼容格式，请关闭浏览器直连 ASR 后改走 proxy，或重新录音。",
    );
  }
  const endpoint = assertDashScopeFunAsrDirectAllowed(provider);
...
function assertFunAsrDirectAudioMime(mimeType: string) {
  if (FUN_ASR_HTTP_AUDIO_MIME_TYPES.has(mimeType)) return;
  throw new DailyApiError(422, "Fun-ASR-Realtime HTTP 接口仅支持 WAV 或 MP3 音频；请重新录音。");
}
